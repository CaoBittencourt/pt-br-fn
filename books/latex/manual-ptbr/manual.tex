...
O efeito, ao longo dos séculos, é a redução vocálica: as vogais não tônicas degeneram a suas correlatas, tendo o ``e'' por correspondente o ``i''; e o ``o'', o ``u'' [u\textprimstress wi; ju\textprimstress wo, wu]\footnote{
    Talvez tenha-se achado engraçado essa frase, porque ``soa chinês''. É a própria regra e o exemplo: o ``i'' e o ``u'' [u\textprimstress wi ju\textprimstress wu] são as semivogais por excelência (esta frase também); e o ``e'' e o ``o'' [iw\textprimstress e ju\textprimstress wo] (também esta), seus correspondentes ``mais fortes''. Para entender melhor, diga-se repetidamente ``e, i'', ``o, u'' e perceba-se a similaridade desses pares: uma é pronunciada com a boca mais aberta, abaixo, mas ambas estão na mesma trajetória. Ou, em termos técnicos, o ``i'' é a vogal anterior fechada não arredondada, o ``e'', a (anterior não arredondada) semifechada; o ``u'', a posterior fechada arredondada, e o ``o'', a semifechada. De fato, uma é o deslocamento vertical da outra.
...
% Não à toa no latim, tantas mais palavras eram terminadas em ``is'' e ``us'' (e.g. \textit{panis et circus}) \textit{vis-à-vis} ``e'' e ``o'' (e.g. \textit{bene}, ): o idioma, em desenvolvimento desde 700 a.C., alcançara seu estado de maturidade na era clássica; e, consequentemente, a ``lei da gravidade vocálica'' já havia agido por séculos.
% Não à toa no latim, tantas mais palavras eram terminadas em ``is'' e ``us'' (e.g. \textit{panis et circus}) \textit{vis-à-vis} ``e'' e ``o'', excetuando-se, obviamente, as declinações (e.g. \textit{bonus}, \textit{bene}, \textit{bono}): o idioma, em desenvolvimento desde 700 a.C., alcançara seu estado de maturidade na era clássica; e, consequentemente, a ``lei da gravidade vocálica'' já havia agido por séculos.
...
% Não à toa no latim, tantas mais palavras eram terminadas em ``is'' e ``us'' (e.g. \textit{panis et circus}) \textit{vis-à-vis} ``e'', ``o'', excetuando-se, obviamente, as declinações (e.g. \textit{bonus}, \textit{bene}, \textit{bono}): o idioma, em desenvolvimento desde 700 a.C., alcançara seu estado de maturidade na era clássica; e, consequentemente, a ``lei da gravidade vocálica'' já havia agido por vários séculos. 
...
\\
        {[}\textprimstress tive \textprimstress nauzea no ko\textprimstress l\textepsilon \textyogh io! \textprimstress dame um \textprimstress k\textopeno po \textprimstress dagua e va a fa\textfishhookr \textprimstress masia com\textprimstress pra\textfishhookr me he\textprimstress m\textepsilon dio{]}\footnote{
            O que pronunciaríamos {[}\textprimstress txivi \textprimstress nawzja nu ko\textprimstress l\textepsilon \textyogh ju! \textprimstress dami ũ \textprimstress k\textopeno po \textprimstress dagwa i va a fah\textprimstress masja co\~w\textprimstress prahmi he\textprimstress m\textepsilon \texttoptiebar{d\textyogh}jo{]}.
        }.
    \end{tabularx}
\end{center}

Por isso, toda e qualquer palavra esdrúxula aparente degenera à sua versão semivocálica: é inevitável. Novamente, no latim, de onde vêm esses ditongos longos, constata-se que não sobreviveram aos séculos: como previsto pela ``lei da gravidade vocálica'' acima, o romano \textit{hodie} [\textprimstress h\textopeno di\textsecstress e], por assim dizer, estava fadado a degerar-se ao ``hoje'' [\textprimstress o\textyogh i], muito mais simples. 

Como o galego conseguiu temporariamente reverter esse processo, é um mistério: 

% Não à toa no latim, de onde vêm esses ditongos longos, tantas palavras eram terminadas em ``is'' e ``us'' (e.g. \textit{panis}, \textit{dominus}). Afinal, o idioma, em desenvolvimento desde 700 a.C, já havia alcançado o seu estado de maturidade na era clássica; e, consequentemente, a ``lei da gravidade vocálica'' agira por séculos e séculos. Mais algum tempo e os ditongos longos provavelmente teriam desaparecido.
% Não à toa no latim, de onde vêm esses ditongos longos, tantas palavras eram terminadas em ``is'' e ``us'' (e.g. \textit{panis et circus}): o idioma, em desenvolvimento desde 700 a.C., alcançara seu estado de maturidade na era clássica; e, consequentemente, a ``lei da gravidade vocálica'' já havia agido por séculos e séculos.

Pela ``lei da gravidade vocálica'', o romano \textit{hodie} [\textprimstress h\textopeno di\textsecstress e]
estava fadado a degerar-se a ``hoje'' [\textprimstress o\textyogh i].

versão do antigo ditado:
\begin{center}
    \begin{tabularx}{\linewidth}{p{0.9\linewidth}}
        ``[\textprimstress agua] longa e língua rápida, tanto fala até que torna-se semivogal [\textprimstress agwa].''
    \end{tabularx}
\end{center}
