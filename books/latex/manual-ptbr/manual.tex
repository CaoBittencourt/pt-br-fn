...
O primeiro fonema é comum nas regiões litorâneas, desde o Nordeste, ao Rio de Janeiro, chegando até à capital de Santa Catarina. O segundo prevalece entre os paulistas e sulistas (principalmente gaúchos). E o restante do país utiliza um ``r'' de ``caipira'' [\textturnr], a aproximante alveolar sonora, ou o omite inteiramente.

Já comentou-se a pronúncia do ``h'' e do ``r'' no Capítulo \ref{pt.section.abc}; e o fonema nulo não precisa de explicação. Basta explicar, então, como o ``hr'' produz o som do ``r'' interiorano. De novo, não trata-se de ``convenção'': qual o ``lly'', é por um mecanismo puramente físico que o ``hr'' serve para representar o fonema [\textturnr]. 
