...
Em particular, para evitar excessivos acentos, opta-se por não explicitar graficamente quando o fonema é oposto ao padrão\footnote{Geralmente, são agudas as letras ``a'', ``i'', ``u'', ``y'' e ``w''; e graves o ``e'' e ``o''.} (viz. o mais frequente), reservando os diacríticos sobretudo para indicar tonicidade (som fraco vs forte), não a altura (som agudo vs grave).
